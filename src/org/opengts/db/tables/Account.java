    ...
    implements UserInformation
{
  ...
  //public static final String FLD_newUserRoleID            = "newUserRoleID";
    ...
    **/
    ...
    /**
    *** Count old events for all devices within this account.
    *** @param acctID The Account ID for which events will be counted.
    *** @param oldTimeSec  The Epoch time before which all events will be counted.
    *** @param log True to print the progress to the logging output.
    **/
    public static long countOldEvents(String acctID, long oldTimeSec, boolean log)
        throws DBException
    {
        String groupID = DeviceGroup.DEVICE_GROUP_ALL;
        if (log) Print.sysPrintln("Counting old events for account "+acctID+" prior to "+(new DateTime(oldTimeSec)));
    ...
    **/
    ...
    /**
    *** Delete old events for all devices within this account.
    *** @param acctID The Account ID for which events will be deleted.
    *** @param oldTimeSec  The EPoch time before which all events will be deleted.
    ***          If there are no events for a device after this specified time, then 
    ***          the most recent event prior to the specified time will be retained.
    *** @param log True to print the progress to the logging output.
    **/
    public static long deleteOldEvents(String acctID, long oldTimeSec, boolean log)
        throws DBException
    {
        String groupID = DeviceGroup.DEVICE_GROUP_ALL;
        if (log) Print.sysPrintln("Deleting old events for account "+acctID+" prior to "+(new DateTime(oldTimeSec)));
    ...
    private static void usage()
    {
        ...
        Print.sysPrintln("  -account=<id>               Account ID to delete/create/edit");
        Print.sysPrintln("");
        Print.sysPrintln("  -delete                     Delete specified Account and owned Devices");
        ...
        Print.sysPrintln("");
        Print.sysPrintln("  -list[=AcctID]              List all Accounts and owned Devices");
        Print.sysPrintln("");
        Print.sysPrintln("  -findEMail=EmailAddr        List all Accounts and owned Devices");
        Print.sysPrintln("");
        Print.sysPrintln("  -countOldEvents=<time>      Count events (for all Devices) before specified time (requires '-confirm')");
        Print.sysPrintln("  -deleteOldEvents=<time>     Delete events (for all Devices) before specified time (requires '-confirm')");
            ...
            try {
                if (deleteEvents) {
                    ...
                    for (String A : acctList) {
                        Account.deleteOldEvents(A, oldTimeSec, true);
                    }
                } else {
                    ...
                    for (String A : acctList) {
                        Account.countOldEvents(A, oldTimeSec, true);
                    }
                }
                System.exit(0);
            } catch (DBException dbe) {
