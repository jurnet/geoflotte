    ...
    **/
    public static void main(String argv[])
    {
        ...
        if(RTConfig.hasProperty("merge")) {
            // merge source
            File merge = RTConfig.getFile("merge",null);
            if (!FileTools.isFile(merge)) {
                Print.sysPrintln("ERROR - 'merge' file does not exist: " + merge);
                System.exit(1);
            }
            // merge destination
            File into  = RTConfig.getFile("into" ,null);
            if (!FileTools.isFile(into)) {
                Print.sysPrintln("ERROR - 'into' file does not exist: " + into);
                System.exit(1);
            }
            // loop through source properties and apply to destination
            RTProperties mergeP = new RTProperties(merge);
            try {
                mergeP.saveProperties(into,true/*merge*/);
            } catch (IOException ioe) {
